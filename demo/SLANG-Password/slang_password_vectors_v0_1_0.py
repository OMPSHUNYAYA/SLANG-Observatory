#!/usr/bin/env python3
"""
SLANG-Password frozen conformance-vector generator and verifier.
...
VECTOR_SCHEMA = "SLANG-PASSWORD-VECTORS-1"
EXPECTED_CORE_FILENAME = "slang_password_v0_1_0.py"
EXPECTED_VECTOR_FILENAME = "SLANG_Password_Vectors_v0_1_0.json"
VECTOR_SET_ID_PREFIX = "slang_password_vector_set_sha256:"


def load_reference(path: Path):
    spec = importlib.util.spec_from_file_location("slang_password_reference", path)
...
def build_disagreement(se) -> Dict[str, Any]:
    value = se.build_multi_verifier_input()
    value["verifier_evidence"][1]["verification_result"] = se.OUTCOME_NO_MATCH
    value["verifier_evidence"][1]["evidence_commitment"] = se.commitment("VECTOR-DISAGREEMENT")
    return refresh(se, value)


def build_missing_verifier(se) -> Dict[str, Any]:
    value = se.build_multi_verifier_input()
    value["verifier_evidence"] = value["verifier_evidence"][:1]
    return refresh(se, value)


def build_unexpected_verifier(se) -> Dict[str, Any]:
    value = se.build_reference_input()
    context = value["context"]
    value["verifier_evidence"].append(
        se.build_evidence_record("VERIFIER-B", "EVIDENCE-002", se.OUTCOME_MATCH, context)
    )
    return refresh(se, value)
...
def build_duplicate_evidence_id(se) -> Dict[str, Any]:
    value = se.build_multi_verifier_input()
    value["verifier_evidence"][1]["evidence_id"] = value["verifier_evidence"][0]["evidence_id"]
    return refresh(se, value)


def build_duplicate_verifier_id(se) -> Dict[str, Any]:
    value = se.build_multi_verifier_input()
    value["verifier_evidence"][1]["verifier_id"] = value["verifier_evidence"][0]["verifier_id"]
    return refresh(se, value)


def build_binding_mismatch(se, field: str, replacement: str) -> Dict[str, Any]:
    value = se.build_reference_input()
    value["verifier_evidence"][0][field] = replacement
    return refresh(se, value)
...
def build_changed_commitment(se) -> Dict[str, Any]:
    value = se.build_reference_input()
    value["verifier_evidence"][0]["evidence_commitment"] = se.commitment("VECTOR-DIFFERENT-EVIDENCE")
    return refresh(se, value)
...
def build_reference_value_input(se) -> Dict[str, Any]:
    ...
    value["verifier_evidence"][0]["subject_ref"] = "HUNTER2"
    return refresh(se, value)
...
def build_ascii_trimmed_identifier_input(se) -> Dict[str, Any]:
    ...
    value["verifier_evidence"][0]["subject_ref"] = " subject-alpha\t"
    return refresh(se, value)


def build_ascii_trimmed_commitment_input(se) -> Dict[str, Any]:
    value = se.build_reference_input()
    current = value["verifier_evidence"][0]["evidence_commitment"]
    value["verifier_evidence"][0]["evidence_commitment"] = " \t" + current.upper() + "\r\n"
    return refresh(se, value)
...
def build_semantic_inputs(se) -> List[Tuple[str, Any]]:
    ...
    no_match = se.build_reference_input(se.OUTCOME_NO_MATCH)
    hidden = se.build_reference_input(visible=False)
    multi_match = se.build_multi_verifier_input(se.OUTCOME_MATCH)
    multi_no_match = se.build_multi_verifier_input(se.OUTCOME_NO_MATCH)
    ...
    normalized["verifier_evidence"][0]["subject_ref"] = " subject-alpha\t"
    ...
    unsupported_verifier_profile = without_declared_identities(
        mutate(
            reference,
            lambda value: value["verifier_evidence"][0].__setitem__(
                "verifier_profile_id", "OTHER-VERIFIER-PROFILE"
            ),
        )
    )

    vectors: List[Tuple[str, Any]] = [
        ("reference_match_visible", reference),
        ("reference_no_match_visible", no_match),
        ("reference_match_withheld", hidden),
        ("multi_verifier_match", multi_match),
        ("multi_verifier_no_match", multi_no_match),
        ...
        ("missing_verifier_evidence", mutate(reference, lambda value: value.pop("verifier_evidence"))),
        ("empty_verifier_evidence", mutate(reference, lambda value: value.__setitem__("verifier_evidence", []))),
        ...
        ("missing_context_request_ref", mutate(reference, lambda value: value["context"].pop("request_ref"))),
        ...
        ("missing_evidence_challenge_ref", mutate(reference, lambda value: value["verifier_evidence"][0].pop("challenge_ref"))),
        ("missing_evidence_verification_result", mutate(reference, lambda value: value["verifier_evidence"][0].pop("verification_result"))),
        ...
        ("credential_version_mismatch", build_binding_mismatch(se, "credential_version", "VERSION-999")),
        ("request_binding_mismatch", build_binding_mismatch(se, "request_ref", "REQUEST-999")),
        ("relying_party_binding_mismatch", build_binding_mismatch(se, "relying_party_ref", "RELYING-PARTY-OTHER")),
        ("challenge_binding_mismatch", build_binding_mismatch(se, "challenge_ref", "CHALLENGE-999")),
        ("multi_verifier_result_disagreement", build_disagreement(se)),
        ("multi_verifier_missing_expected_verifier", build_missing_verifier(se)),
        ("single_verifier_unexpected_verifier", build_unexpected_verifier(se)),
        ("duplicate_evidence_id", build_duplicate_evidence_id(se)),
        ("duplicate_verifier_id", build_duplicate_verifier_id(se)),
        ...
        ("unsupported_verifier_profile", unsupported_verifier_profile),
        ...
        ("unknown_evidence_field", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("unexpected", "VALUE")))),
        ...
        ("invalid_evidence_identifier", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("evidence_id", "INVALID SPACE")))),
        ("invalid_evidence_commitment", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("evidence_commitment", "sha256:1234")))),
        ("invalid_verification_result", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("verification_result", "UNKNOWN")))),
        ...
        ("verifier_evidence_not_array", without_declared_identities(mutate(reference, lambda value: value.__setitem__("verifier_evidence", {})))),
        ("context_not_object", without_declared_identities(mutate(reference, lambda value: value.__setitem__("context", [])))),
        ("evidence_record_not_object", without_declared_identities(mutate(reference, lambda value: value.__setitem__("verifier_evidence", ["EVIDENCE"])))),
        ...
        ("single_mode_multiple_expected_verifiers", without_declared_identities(mutate(reference, lambda value: value["context"].__setitem__("expected_verifier_ids", ["VERIFIER-A", "VERIFIER-B"])))),
        ("multi_mode_one_expected_verifier", without_declared_identities(mutate(multi_match, lambda value: value["context"].__setitem__("expected_verifier_ids", ["VERIFIER-A"])))),
        ("empty_expected_verifier_ids", without_declared_identities(mutate(reference, lambda value: value["context"].__setitem__("expected_verifier_ids", [])))),
        ("duplicate_expected_verifier_ids", without_declared_identities(mutate(multi_match, lambda value: value["context"].__setitem__("expected_verifier_ids", ["VERIFIER-A", "VERIFIER-A"])))),
        ...
        ("conflict_precedence_over_unsupported", mutate(reference, lambda value: (value["verifier_evidence"][0].__setitem__("subject_ref", "SUBJECT-OTHER"), value.__setitem__("profile_id", "OTHER-PROFILE")))),
        ...
        ("commitment_trailing_nel_unsupported", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("evidence_commitment", value["verifier_evidence"][0]["evidence_commitment"] + "\u0085")))),
        ("commitment_trailing_nbsp_unsupported", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("evidence_commitment", value["verifier_evidence"][0]["evidence_commitment"] + "\u00a0")))),
        ("commitment_non_ascii_character_unsupported", without_declared_identities(mutate(reference, lambda value: value["verifier_evidence"][0].__setitem__("evidence_commitment", "sha256:" + ("a" * 63) + "ａ")))),
    ...
    return [
        {
            "name": name,
            "input": value,
            "expected": se.resolve_password(value),
        }
...
def build_presentation_vectors(se) -> List[Dict[str, Any]]:
    values = [
        ("visible_match", se.build_reference_input()),
        ("visible_no_match", se.build_reference_input(se.OUTCOME_NO_MATCH)),
        ("withheld_match", se.build_reference_input(visible=False)),
...
def build_relations(se) -> List[Dict[str, Any]]:
    ...
    no_match = se.build_reference_input(se.OUTCOME_NO_MATCH)
    hidden = se.build_reference_input(visible=False)
    multi = se.build_multi_verifier_input()
    ...
    return [
        ...
        },
        {
            ...
            "left_input": reference,
            "right_input": mutate(reference, lambda value: value["verifier_evidence"].__setitem__(0, reordered_object(value["verifier_evidence"][0]))),
            "equal_fields": ["canonical_input_id", "evidence_set_id", "result_id"],
            "different_fields": [],
        },
        {
            "name": "multi_verifier_record_order_invariance",
            "left_input": multi,
        ...
        },
        {
            ...
            "left_input": reference,
            "right_input": hidden,
            "equal_fields": ["state", "verification_outcome", "admission_state", "evidence_set_id"],
            "different_fields": ["visibility_state", "outcome_visible", "outcome_id", "result_id"],
        },
        {
            "name": "match_no_match_distinction",
            "left_input": reference,
            "right_input": no_match,
            "equal_fields": ["state", "context_id", "verifier_manifest_id", "rule_profile_id"],
            "different_fields": ["verification_outcome", "admission_state", "evidence_set_id", "outcome_id", "result_id"],
        },
        {
            "name": "request_identity_change",
            "left_input": reference,
            "right_input": build_changed_context(se, "request_ref", "REQUEST-NEW"),
            "equal_fields": ["state", "verification_outcome", "admission_state"],
            "different_fields": ["context_id", "canonical_input_id", "outcome_id", "result_id"],
        },
        {
            "name": "credential_version_identity_change",
            "left_input": reference,
            "right_input": build_changed_context(se, "credential_version", "VERSION-004"),
            "equal_fields": ["state", "verification_outcome", "admission_state"],
            "different_fields": ["context_id", "canonical_input_id", "outcome_id", "result_id"],
        },
        {
            ...
            "equal_fields": ["state", "verification_outcome", "admission_state", "context_id"],
            "different_fields": ["evidence_set_id", "evidence_agreement_id", "canonical_input_id", "result_id"],
        },
        {
            "name": "raw_secret_changes_state",
            "left_input": reference,
            "right_input": mutate(reference, lambda value: value.__setitem__("password", "VECTOR-RAW-PASSWORD")),
            "equal_fields": ["execution_authority", "authentication_authority", "access_authority", "session_authority"],
...
def build_artifact_vectors(se) -> List[Dict[str, Any]]:
    ...
    authority_receipt = copy.deepcopy(reference_receipt)
    authority_receipt["authentication_authority"] = "LOGIN"
    authority_receipt["receipt_id"] = se.identity(
        se.RECEIPT_ID_PREFIX,
        {key: value for key, value in authority_receipt.items() if key != "receipt_id"},
    )

    unrelated_receipt = se.make_receipt(se.build_bundle(se.build_reference_input(se.OUTCOME_NO_MATCH)))
    ...
    return [
        ...
        {"name": "authority_receipt_rejected", "operation": "VERIFY_RECEIPT", "receipt": authority_receipt, "expected_ok": False, "expected_reason": "AUTHENTICATION_AUTHORITY_MISMATCH"},
...
def verify_document(se, document: Dict[str, Any]) -> Tuple[Dict[str, Tuple[int, int]], List[str]]:
    ...
    for vector in semantic_vectors:
        ...
        if not isinstance(vector, dict):
            ...
            continue
        actual = se.resolve_password(vector.get("input"))
    ...
    for relation in relations:
        ...
        if not isinstance(relation, dict):
            ...
            continue
        left = se.resolve_password(relation.get("left_input"))
        right = se.resolve_password(relation.get("right_input"))
        equal_ok = all(
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SLANG-Password frozen conformance vectors")
